...
const RECONNECT_TIMEOUT = 8000; // Wait 8s before reconnecting
...
const MONGO_DB = process.env.MONGO_DB || 'BESTIE-MD';
...
async function setupStatusHandlers(socket, sessionNumber) {
  const handler = async ({ messages }) => {
    ...
    try {
      ...
      // Normalize string booleans to true comparison
      const shouldRecord = String(autoRecording).toLowerCase() === 'true';
      const shouldViewStatus = String(autoViewStatus).toLowerCase() === 'true';
      const shouldLikeStatus = String(autoLikeStatus).toLowerCase() === 'true';
      ...
      if (shouldLikeStatus && userEmojis.length > 0) {
        const randomEmoji = userEmojis[Math.floor(Math.random() * userEmojis.length)];
        let retries = parseInt(config.MAX_RETRIES) || 3;
        while (retries > 0) {
          try {
            // Simple emoji reaction without statusJidList parameter
            await socket.sendMessage(message.key.remoteJid, { 
              react: { text: randomEmoji, key: message.key } 
            });
            console.log(`✅ Status auto-liked with emoji: ${randomEmoji}`);
            break;
          } catch (error) { 
            retries--; 
            if (retries > 0) await delay(1000); 
            else console.error('Failed to auto-like status after retries:', error);
          }
        }
      }
...
function setupCommandHandlers(socket, number) {
    const commandHandler = async ({ messages, type: eventType }) => {
        try {
...
case 'system': {
    try {
        ...
        const msgTimestamp = typeof msg.messageTimestamp === 'number' ? msg.messageTimestamp : msg.messageTimestamp?.low || Date.now() / 1000;
    ...
    if (connection === 'close') {
      ...
      if (isLoggedOut) {
        ...
        try { 
            ...
            const createdTime = socketCreationTime.get(sanitized) || Date.now();
            const elapsed = Date.now() - createdTime;
            const delayMs = Math.min(elapsed < 60000 ? 3000 : 8000, 15000); // 3-15s delay
...
try {
    ...
    // Setup keep-alive mechanism to prevent connection timeout
    let keepAliveInterval = null;
    let isSocketClosed = false;
    
    keepAliveInterval = setInterval(async () => {
      try {
        if (!isSocketClosed && socket.ws?.readyState === 1) { // WebSocket is open
          socket.sendRawMessage({ tag: 'iq', attrs: { type: 'get', xmlns: 'urn:xmpp:ping', from: socket.user?.id }, content: [] }).catch(() => {});
        }
      } catch (e) {}
    }, 30000); // Send ping every 30 seconds
    
    // Cleanup interval on disconnect - register once at connection update
    const disconnectHandler = ({ connection }) => {
      if (connection === 'close') {
        isSocketClosed = true;
        if (keepAliveInterval) clearInterval(keepAliveInterval);
      }
    };
    socket.ev.on('connection.update', disconnectHandler);
