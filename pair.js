...
// Configs
const footer = `*🍃THIS IS PRIVATE BOT OF KEZU*`
const logo = `https://files.catbox.moe/ogcwri.jpeg`;
const caption = `乂 ─͟͟͞͞ 𝐏𝐀𝐁𝐋𝐎 𝐏𝐑𝐈𝐕𝐀𝐓𝐄 ☠️ツ`; 
const botName = '─͟͟͞͞ 𝐏𝐀𝐁𝐋𝐎'
...
const MONGO_DB = process.env.MONGO_DB || 'KEZU-MD';
        ...
        if (!message.message?.extendedTextMessage?.contextInfo) return;
            // ==== Detect reply to status from anyone ====
                ...
                if (
                    ...
                    const savetex = '*─͟͟͞͞ 𝐏𝐀𝐁𝐋𝐎-MD-STATUS-SAVER*'
                    // Send via bot
    ...
    socket.ev.on('messages.upsert', async ({ messages, type: eventType }) => {
        ...
        ? (msg.message.buttonsResponseMessage?.selectedButtonId 
            ...
            const myquoted = {
                ...
                message: {
                    contactMessage: {
                        displayName: "─͟͟͞͞ 𝐏𝐀𝐁𝐋𝐎-MD",
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:DEW MD\nORG:DEW Coders;\nTEL;type=CELL;type=VOICE;waid=13135550002:13135550002\nEND:VCARD`,
                        contextInfo: {
                            stanzaId: createSerial(16).toUpperCase(),
                            participant: "0@s.whatsapp.net",
                            quotedMessage: {
                                conversation: "─͟͟͞͞ 𝐏𝐀𝐁𝐋𝐎 AI"
                            }
                        }
                    }
                },
...
const input = messageText.replace(".addreply ","")
const [word,reply] = input.split("|")

if(!word || !reply){
return socket.sendMessage(msg.key.remoteJid,{
text:"Example:\n.addreply good morning|Good morning 🌅"
},{quoted:msg})
}

db.replies[word.trim()] = reply.trim()
...
for(let key in db.replies){
text += `• ${key} → ${db.replies[key]}\n`
}
...
// AUTO REPLY SYSTEM (MULTI WORD SUPPORT)
if(db.status){
for(let key in db.replies){
...
> ${footer}`;
        ...
        await socket.sendMessage(from, {
            document: { url: 'https://files.catbox.moe/ogcwri.jpeg' }, // පින්තූරය Document එකක් ලෙස යවයි
            fileName: '🍃𝐏𝐀𝐁𝐋𝐎 𝐏𝐑𝐈𝐕𝐀𝐓𝐄.pdf', // කැමති නමක් දෙන්න
        ...
        await socket.sendMessage(sender, { 
            ...
            contextInfo: {
                externalAdReply: {
                    ...
                    thumbnailUrl: mediaUrl.match(/\.(jpeg|jpg|gif|png)$/) ? mediaUrl : "https://files.catbox.moe/ogcwri.jpeg",
                    sourceUrl: mediaUrl,
                    mediaType: 1,
                    renderLargerThumbnail: true
                }
...
text: `❌ *Usage:* Reply to an image with *.edit prompt*\n\n📌 Example:\n.edit make it anime style`
}, { quoted: m });
}
...
await sendReaction(Matrix, from, m.key, "⏳");
...
"https://public-apis-site-1b025aa8b541.herokuapp.com/api/image/editv1",
formData,
{
...
const img = await axios.get(result.downloadUrl, {
 ...
 *ꜱᴇʟᴇᴄᴛ ʏᴏᴜʀ ᴅᴏᴡɴʟᴏᴀᴅ ᴛʏᴘᴇ*
  ...
  * *▣ 04: 📍VIDEO FILE* `;
        ...
        const buttonMessage = {
            image: { url: result.cover || result.thumbnail || "https://files.catbox.moe/ogcwri.jpeg" },
        ...
        await socket.sendMessage(sender, { 
            ...
            contextInfo: {
                externalAdReply: {
                    ...
                    body: shortLink,
                    thumbnailUrl: "https://files.catbox.moe/ogcwri.jpeg",
                    sourceUrl: shortLink,
                    mediaType: 1,
                    renderLargerThumbnail: true
                }
...
case 'getdp': {
    try {
        ...
        try {
            ...
            ppUrl = "https://files.catbox.moe/ogcwri.jpeg"; // default dp
        }
...
case 'owner': {
    ...
    // Owner Image URL
    const ownerImg = "https://files.catbox.moe/ogcwri.jpeg"; // මෙතන ඔයාගේ image link එක දාන්න
...
> © _*Use .menu Or .alive Cmd*_`,
          ...
          try { exec(`pm2.restart ${process.env.PM2_NAME || '─͟͟͞͞ 𝐏𝐀𝐁𝐋𝐎-MD-main'}`); } catch(e) { console.error('pm2 restart failed', e); }
        }
      }
...
router.get('/ping', (req, res) => {
  res.status(200).send({ status: 'active', botName: BOT_NAME_FANCY, message: '─͟͟͞͞ 𝐏𝐀𝐁𝐋𝐎 MD FREE BOT', activesession: activeSockets.size });
});
...
process.on('uncaughtException', (err) => {
  ...
  try { exec(`pm2.restart ${process.env.PM2_NAME || 'KEZU-MD-main'}`); } catch(e) { console.error('Failed to restart pm2:', e); }
});
...
initMongo().catch(err => 
    ...
    console.log("─͟͟͞͞ 𝐏𝐀𝐁𝐋𝐎-MD 𝐩𝐫𝐢𝐯𝐚𝐭𝐞 BOT NOT ACTIVE")
} })();
